...
test("ssh client sends expected request shapes", async () => {
  ...
  assert.equal(calls[1]?.path, "/v1/sandbox/sb-1/ssh/ssh%2F1/validate");
  assert.deepEqual(jsonOf(calls[1]!), { password: "secret" });
  assert.equal(calls[2]?.path, "/v1/sandbox/sb-1/ssh/ssh%2F1/regen");
  assert.equal(calls[3]?.path, "/v1/sandbox/sb-1/ssh/ssh%2F1");
});
